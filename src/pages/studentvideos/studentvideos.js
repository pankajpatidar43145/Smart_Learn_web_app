...
function TimestampsPage() {
  ...
  useEffect(() => {
    // Make a GET request to fetch the email data
    const token = localStorage.getItem("temptoken");

    // Check if token exists before decoding
    const decodedToken = decodeJwt(token);
    emailtemp = decodedToken.payload.email;
  }, []);

  useEffect(() => {
    // Fetch data from the API using the extracted email
    fetch(
      `https://smart-learn-web-app-u8vp.onrender.com/timestamps/getTimestampsByEmail?email=${emailtemp}`
    )
      .then((response) => {
        if (!response.ok) {
  ...
  const navigateToTimestamp = (index) => {
    ...
    window.location.href = `/addtimestamp/${index}`;
  };
    ...
    <div>
      ...
      <div style={backgroundStyle}>
        <Link to="/studenthome">
          ...
          <button className="student-video-back-button">
            {" "}
